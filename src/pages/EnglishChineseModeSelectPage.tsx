...
export default function EnglishChineseModeSelectPage() {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const mode = searchParams.get('mode') || 'english_chinese';
  ...
  const modeTitle = mode === 'english_chinese' 
    ? '英汉模式' 
    : mode === 'chinese_english' 
      ? '汉英模式' 
      : '混合模式';

  const modeDescription = mode === 'english_chinese'
    ? '看英文选中文'
    : mode === 'chinese_english'
      ? '看中文填英文'
      : '随机出现英汉或汉英题目';
